...
public class EmployeeController : ControllerBase
{
    ...
    public IActionResult GetActiveEmployeesAttendance([FromQuery] string fromDate, [FromQuery] string toDate)
    {
    ...
    //[HttpPost("pos/")]
    //public IActionResult InsertEmployeePurchase([FromBody] List<EmpPosDto> empPosList)
    //{
    //    using (var connection = new OracleConnection(_connectionString))
    //    {
    //        connection.Open();
    ...
    //            {
    //                string query = @"
    //                INSERT INTO EMP_POS (EMP_ID, EMP_CODE, AMOUNT, PURCHASE_DATE)
    //                VALUES (:empId, :empCode, :amount, TO_DATE(:purchaseDate, 'YYYY-MM-DD'))";

    //                foreach (var empPos in empPosList)
    //                {
    //                    using (var command = new OracleCommand(query, connection))
    //                    {
    //                        command.Parameters.Add(new OracleParameter("empId", empPos.EMP_ID));
